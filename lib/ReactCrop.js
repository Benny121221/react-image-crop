...
class ReactCrop extends PureComponent {
  ...
  getRulerIncrements(image){
      var realWidth = image.naturalWidth;
      var displayedWidth = image.width;
      var increments=[]
      for(var i = 0; i < realWidth / 100; i++){
          increments[i] = (<li key={i} style={{
              display: "inline-block",
              width: `${(displayedWidth / realWidth) * 100}px`,
              borderLeft: "1px solid grey",
              height: "30px"
            }}>{i * 100}</li>);
      }
      return <ul style={{ padding: "0px" }}>{increments}</ul>;
  }
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        {ruler ? (<div className="ruler">{this.imageRef ? this.getRulerIncrements(this.imageRef) : null}</div>) : null}
        {children}
        {cropSelection}
      </div>
    );
